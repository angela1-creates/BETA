  ...
  return <div className="app-shell">
    ...
    {screen === 'workspace' && <main className="workspace-page">
      <div className="workspace-top">
        ...
        <div className="route-heading"><div><span className="eyebrow">{route.grade} · Route study</span><h1>{route.title}</h1><p>{route.notes}</p></div>
...
function Library({ onOpen, attempts }: { onOpen: (r: Route) => void; attempts: Record<string, Attempt> }) {
  ...
  const upload = (file?: File) => {
    ...
    reader.onload = () => onOpen({
      ...
      grade: 'Open', accent: '#315c4d', notes: 'A route from your own wall.', insight: 'Start with what catches your attention.',
  ...
  return <main className="page library-page"><div className="page-intro library-intro"><div><span className="eyebrow">Route library · 23 studies</span><h1>What do you notice?</h1><p>Choose a line, study the holds, then sketch the movement you imagine.</p></div>{completed > 0 && <span className="local-progress">{completed} saved locally</span>}</div>
    ...
    <div className="route-grid">{routes.map((r, i) => <button className={`route-card ${i < 3 ? 'featured' : ''}`} key={r.id} onClick={() => onOpen(r)}><div className="route-image" style={{ '--accent': r.accent, '--shift': `${(i % 5) * 3}%` } as React.CSSProperties}><img src={r.image} alt="" /><span className="grade">{r.grade}</span>{attempts[r.id]?.predictedBeta.length ? <span className="saved"><Icon name="check" size={14} /> Saved</span> : null}</div><div className="route-card-copy"><span>{String(i + 1).padStart(2, '0')}</span><div><h2>{r.title}</h2><p>{r.insight}</p></div><b>↗</b></div></button>)}</div>
  </main>
}
